...
Original file is located at
    https://colab.research.google.com/drive/18ZMZzL7rBhtJB-1t1oajl887tzN8SaLC
"""

!pip install sentence_transformers chromadb langchain langchain-community spacy openai
    ...
    if response.status_code == 200:
        ...
        if "search" in data and data["search"]:
            for entry in data["search"]:
                #print(entry["display"]["label"]["value"])
...
def identify_items_props(query, entities):
    ...
    # Initialize the LLM agent
    llm = OpenAI(temperature=0, api_key=openai_api_key)  # Make sure you have set up OpenAI API key as an environment variable
...
def query_generator(query):
    ...
    possible_properties = result["Properties"]
